...
SDL_FRect srcwalk[6]={
  {32,32,32,32},{64,32,32,32},{96,32,32,32},
...
void walk(SDL_Renderer* renderer){
   ...
   if(flip){
  SDL_RenderTextureRotated(renderer,player,&srcwalk[walk_frame],&dstplayer,0,NULL,SDL_FLIP_HORIZONTAL);
   }else{
...
void movement(){
 ...
 const bool *keystate = SDL_GetKeyboardState(NULL);
  ...
  if(dstplayer.x>=800-dstplayer.w){
    dstplayer.x=800-dstplayer.w;
  }
  if(dstplayer.x<0){
    dstplayer.x=0;
  }
  if(dstplayer.y>=800-dstplayer.h){
    dstplayer.y=800-dstplayer.h;
  }
  if(dstplayer.y<0){
    dstplayer.y=0;
  }
...
bool coliderrect(SDL_FRect rect1,SDL_FRect rect2){
     if(rect1.x<rect2.x+rect2.w &&rect1.x+rect1.w>rect2.x && rect1.y<rect2.y+rect2.h && rect1.y+rect1.y>rect2.y){
         SDL_Log("coliderect");  
         return true;
     }     
     return false;
